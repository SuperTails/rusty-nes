...
impl Mapper3 {
    pub fn new(prg_rom: Vec<u8>, chr: Vec<u8>, prg_ram_len: usize) -> Mapper3 {
        ...
        Mapper3 {
            ...
            data: RefCell::new(Mapper3Data::new()),
        }
    }
}

pub enum Mapper3Location<'a> {
    CPU((&'a RefCell<Mapper3Data>, u16, u8)),
...
impl<'a> MemLocation<'a> for Mapper3Location<'a> {
    ...
    fn write(&mut self, value: u8) {
        match self {
            Mapper3Location::CPU((data, addr, _)) => {
...
impl Mapped for Mapper3 {
    fn mem_cpu<'a>(&'a self, addr: u16, _: &'a Context) -> MapperResult<'a> {
        match addr {
            ...
            0x8000..=0xFFFF => Mapper3Location::CPU({
    ...
    fn mem_ppu<'a>(&'a self, addr: u16) -> MapperResult<'a> {
        match addr {
            0x0000..=0x1FFF => {
                ...
                Mapper3Location::PPU((&self.chr, mapped_addr.try_into().unwrap())).into()
            }
            _ => panic!(),
        }
    }
